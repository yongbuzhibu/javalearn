...
public class NIODemo {
    ...
    public static void copyFileWithMMap(String original, String target) {
        try {
            ...
            MappedByteBuffer inputbuffer = input.getChannel().map(FileChannel.MapMode.READ_ONLY, 0, size);
            MappedByteBuffer outBuffer = output.getChannel().map(FileChannel.MapMode.READ_WRITE, 0, size);
